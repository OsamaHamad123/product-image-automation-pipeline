...
# سكربت للتحقق من الاتصال بالخدمات السحابية الأربعة الأساسية وتأكيد صحة الاعتمادات
...
def verify_google_sheets():
    ...
    if not os.path.exists(creds_file):
        print(f"❌ خطأ: ملف الاعتمادات لم يتم العثور عليه في: {creds_file}")
        return False
...
def verify_gemini():
    ...
    # استخدام الموديل الافتراضي في config أو fallback لـ gemini-1.5-flash
    model = getattr(config, "GEMINI_MODEL", "gemini-1.5-flash")
    url = f"https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent?key={api_key}"
    headers = {"Content-Type": "application/json"}
    
    # رسالة فحص خفيفة لتقليل استهلاك الحصة
    payload = {
        "contents": [{
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"❌ فشل الاتصال: كود الاستجابة {response.status_code}")
...
def verify_photoroom():
    ...
    try:
        print("🔄 إرسال طلب فحص أولي لـ PhotoRoom (طلب فارغ للتحقق من المصادقة)...")
        # نرسل طلب فارغ (بدون صورة) للتحقق فقط مما إذا كان المفتاح مقبولاً (سيستجيب بـ 400/415 بدلاً من 401/403)
        response = requests.post(url, headers=headers, timeout=10)
        
        # كود 400 أو 415 يعني أن المصادقة نجحت لكن المدخلات فارغة (وهذا المطلوب للتحقق دون استهلاك رصيد)
...
if __name__ == "__main__":
    ...
    for service, status in results.items():
        status_str = "✅ يعمل بنجاح" if status else "❌ فشل الاتصال / غير مهيأ"
